...
@section('content')
    ...
    <section class="relative h-64 bg-cover bg-center text-white"
        ...
        <div class="relative z-10 flex items-center justify-center h-full text-center">
            <h1 class="font-lora text-5xl font-extrabold tracking-tight">Apa Kata Mereka?</h1>
        </div>
    </section>
                            ...
                            <div class="flex items-start mb-4">
                                ...
                                @if ($testimonial->product)
                                    <a href="{{ route('products.show', $testimonial->product->slug) }}"
                                        class="flex-shrink-0">
                                        <img src="{{ $testimonial->product->images->isNotEmpty() ? asset('storage/' . $testimonial->product->images->first()->path) : 'https://via.placeholder.com/100' }}"
                                            alt="{{ $testimonial->product->name }}"
                                            class="h-16 w-16 object-cover rounded-md">
                                    </a>
                                @else
                                    {{-- Tampilan kalau produk dihapus --}}
                                    <div
                                        class="flex-shrink-0 h-16 w-16 bg-gray-200 rounded-md flex items-center justify-center text-gray-500 text-xs text-center">
                                        Produk Dihapus
                                    </div>
                                @endif
                                ...
                                <div class="ml-4">
                                    ...
                                    @else
                                        <span class="font-semibold text-gray-500 italic">Produk Tidak Tersedia</span>
                                    @endif
