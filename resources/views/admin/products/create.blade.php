    ...
    <div class="bg-white p-6 rounded-lg shadow-sm">
        <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-6">
                        ...
                        @forelse ($categories as $category)
                            <label class="flex items-center text-sm text-slate-600">
                                <input type="checkbox" name="categories[]" value="{{ $category->id }}"
                                    class="h-4 w-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500 mr-2">
                                {{ $category->name }}
                            </label>
                        @empty
                            <p class="text-slate-500 col-span-full text-sm">Belum ada kategori. Silakan buat dulu di menu
                                Kategori.</p>
                        @endforelse
                    </div>
                </div>
                ...
                <div>
                    <label for="images" class="block text-sm font-medium text-slate-700 mb-1">Gambar Produk (Bisa lebih
                        dari satu)</label>
            ...
            <div class="border-t border-slate-200 mt-6 pt-6 flex items-center gap-4">
                <button type="submit"
                    class="inline-flex items-center justify-center rounded-md border border-transparent bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm transition-colors hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">Simpan
                    Produk</button>
                <a href="{{ route('admin.products.index') }}"
                    class="text-sm font-medium text-slate-600 hover:text-slate-800">Batal</a>
            </div>
        </form>
    </div>
@endsection
