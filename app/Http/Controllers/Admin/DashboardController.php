...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $newCustomersThisMonthCount = User::whereYear('created_at', $now->year)
            ->whereMonth('created_at', $now->month)
            ->count();

        $ordersThisMonthCount = Order::whereYear('created_at', $now->year)
            ->whereMonth('created_at', $now->month)
            ->count();
        ...
        // 8. Hitung Keuntungan Bersih (Net Profit)
        // Estimasi Modal Barang = 70% dari Harga Jual (Margin 30%)
        $totalCostOfGoods = DB::table('orders')
            ->join('order_items', 'orders.id', '=', 'order_items.order_id')
            ->join('products', 'order_items.product_id', '=', 'products.id')
            ->where('orders.status', 'completed')
            ->whereYear('orders.created_at', $now->year)
            ->whereMonth('orders.created_at', $now->month)
            ->sum(DB::raw('order_items.quantity * products.price * 0.7'));

        $netProfitThisMonth = $revenueThisMonth - $totalCostOfGoods - $expensesThisMonth;
